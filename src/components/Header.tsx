...
export default function Header({ onSignIn }: HeaderProps) {
  ...
  return (
    <header className="w-full flex items-center justify-between px-4 py-3 bg-white border-b border-gray-200">
      ...
      {/* Upgrade Modal */}
      {showUpgrade && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40" onClick={() => setShowUpgrade(false)}>
          <div className="relative" onClick={e => e.stopPropagation()}>
            <button
              className="absolute top-4 right-4 text-gray-400 hover:text-gray-700 text-2xl font-bold z-10"
            ...
            {/* You can replace this with your UpgradeModal component if you have one */}
            <div className="bg-white p-8 rounded shadow-lg">
              <h2 className="text-xl font-bold mb-4">Upgrade Required</h2>
              <p className="mb-4">You have used your 2 free credits. Please upgrade to continue using the service.</p>
              <button className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded" onClick={() => setShowUpgrade(false)}>Close</button>
            </div>
          </div>
        </div>
      )}
